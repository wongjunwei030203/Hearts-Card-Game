...
class Round:
    ...
    def __init__(self, players) -> None:
        ...
        Card.pretty_print = True            # Sets Card.pretty_print to True so that when card is printed a text-image can be printed instead of a word string
    ...
    def play_round(self):
        ...
        while (len(self.players[0].hand)) > 0:                                                          # the while loop runs again and again until the self.hand of a player has 0 elements / cards
            ...
            for player_index in range(len(self.players)):                                               # a for loop that runs the number of times as the number of players in self.players

                player = self.players[player_index]                                                     # specific player details (name, hand, roundscore, totalscore) --> each player from self.players
                ...
                if player_index == 0:
                    print(f"{player} leads with {smallest_card_to_play}")                               # prints the player name and the card played for the player who leads the game
                    time.sleep(3)
                else:
            ...
            print("*"*len(f"{self.players[next_player_index]} takes the trick. Points received: {penalty} 😢")+"*")      # prints '*' around the line that shows the player name and penalty recieved
